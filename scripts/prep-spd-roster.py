...
def main():
    ...
    # Split the names out
    df.apply(lambda x: first_name(x), axis=1)
    df.apply(lambda x: middle_name(x), axis=1)
    df.apply(lambda x: last_name(x), axis=1)
...
def first_name(df: DataFrame):
    df['first_name'] = df['full_name'].split(' ')[1].strip()


def middle_name(df: DataFrame):
    name_list = df['full_name'].split(' ')
    if len(name_list) > 2:
        df['middle_name'] = ' '.join(name_list[2:])


def last_name(df: DataFrame):
    df['last_name'] = df['full_name'].split(' ')[0].strip(' ,')
